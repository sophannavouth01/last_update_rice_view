<template>
	<div class="bg-white shadow-md h-screen rounded-lg">
		<p class="p-5 text-lg font-semibold underline text-[#00992B] moul-regular">បញ្ជីអតិថិជនត្រូវប្រមូល</p>
			...
			<table class="w-full table-radius text-sm text-left text-gray-500">
				<thead class="text-xs text-gray-700 bg-gray-100">
					<tr class="text-[14px]">
						...
						<th class="px-6 py-3 text-nowrap border moul-regular">សកម្មភាព</th>
					</tr>
				</thead>
				<tbody>
					<tr v-for="(item, index) in paginatedItems" :key="item.id"
						...
						<td class="flex siemreap-regular border justify-center px-6 py-3 text-center">
							<button @click="confirmUpdate(item.id, 'Paid')" class="px-4 py-2 bg-blue-50 text-center text-[#00992B] rounded-md">ទូទាត់កម្ខី</button>
						</td>
					</tr>
				</tbody>
			</table>
...
const fetchOrders = async () => {
	try {
		...
		localOrders.value = response.data
			.filter(order => order.status.toLowerCase() === 'transfertocustomer' && order.customer && order.paymentStatus === 'Not yet')
			.sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt));
		console.log("Filtered orders with 'TransferToCustomer' status, non-null customer, and 'Not yet' payment status:", localOrders.value);
	} catch (error) {
