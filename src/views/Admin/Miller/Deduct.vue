<template>
	<div class="bg-white shadow-md h-screen rounded-lg">
		<p class="p-5 text-lg font-semibold underline text-[#00992B] moul-regular">ដកអង្ករពីរោងម៉ាសុីន</p>
...
const fetchOrders = async () => {
	try {
		...
		localOrders.value = response.data
			.filter(order => order.status.toLowerCase() === 'deductfrommiller') // Make lowercase for consistency
...
const createRecord = async () => {
	...
	try {
		...
		const payload = {
			...
			cost: newRecord.value.cost || 0, // Set default if necessary
			totalCost: newRecord.value.totalCost || 0, // Set default if necessary
			paymentStatus: newRecord.value.paymentStatus || 'Pending', // Set to default if needed
			section: newRecord.value.section || '1', // Default if necessary
			status: 'deductFromMiller',
			purchaseDate: new Date().toISOString(),
