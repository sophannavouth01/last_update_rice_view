<template>
	<div class="bg-white shadow-md h-screen rounded-lg">
		<p class="p-5 text-lg font-semibold underline text-[#00992B] moul-regular">បែងចែកទៅភ្នាក់ងារ</p>
			...
			<table class="w-full table-radius text-sm text-left text-gray-500">
				<thead class="text-xs text-gray-700 bg-gray-100">
					<tr class="text-[14px]">
						...
						<th class="px-6 py-3 text-nowrap border moul-regular">ភ្នាក់ងារ</th>
                        ...
                        <th class="px-6 py-3 text-nowrap border w-[120px] moul-regular">ពីសាខា</th>
...
const fetchOrders = async () => {
    try {
        ...
        // Filter orders for 'TransferToAgent' status and ensure both `branch` and `agent` are not null
        localOrders.value = response.data
            .filter(order => order.status.toLowerCase() === 'transfertoagent' && order.branch && order.agent)
            .sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt));

        console.log("Filtered and sorted orders with 'TransferToAgent' status, and non-null branch and agent:", localOrders.value);
    } catch (error) {
