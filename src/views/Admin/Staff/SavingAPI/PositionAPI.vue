	...
	<div class="bg-white shadow-md rounded-lg">
			...
			<table class="w-full table-radius   text-sm text-left text-gray-500"
				...
				<thead class="text-xs text-gray-700  bg-gray-100">
					<tr class="text-[14px]">
						...
						<th
							class="border w-[150px] text-center px-6 py-3 whitespace-nowrap overflow-hidden text-ellipsis moul-regular">
							ស្ថានភាព
						</th>
						<th
							class="border px-6 moul-regular w-[150px]  py-3 whitespace-nowrap overflow-hidden text-ellipsis text-center">
							សកម្មភាព
						</th>
					</tr>
				</thead>
				<tbody style="border-radius: 10px">
					<tr v-for="(item, index) in paginatedItems" :key="item.id"
						...
						<td class="px-6 text-center siemreap-regular py-4 border whitespace-nowrap overflow-hidden text-ellipsis"
						...
						<td class="border flex justify-center  py-2 text-center ">
							<button @click="openEditModal(item)"
								class="  px-4 py-2 bg-blue-50 text-center text-[#00992B]  rounded-md">លម្អិត
							</button>
							<button @click="confirmDelete(item.id)"
								class=" px-4 py-2 ml-2 bg-red-50 text-center text-red-600 rounded-md">លុប
							</button>
						</td>
					</tr>
				</tbody>
		...
		<div v-if="isEditModalOpen" class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50">
			<div class="bg-white px-6 py-5 rounded-lg w-1/3">
				<h2 class="text-xl mb-5 text-[#00992B] text-center underline">
					កែប្រែមុខដំណែង
				</h2>
				<form @submit.prevent="handleEditSubmit">
					<div class="mb-4 flex">
						<label class="block text-gray-700 w-[130px] pt-2">ឈ្មោះសិទ្ធ:</label>
						<input v-model="selectedRole.name" type="text" class="w-full border px-3 py-2 rounded-lg"
							required />
					</div>
					<div class="mb-4 flex">
						<label class="block text-gray-700 w-[130px] pt-2">ការពិពណ៌នា:</label>
						<input v-model="selectedRole.description" type="text"
...
const openAddModal = () => {
	role.value = { id: null, name: "", description: "" };
...
const handleAddSubmit = async () => {
	try {
		const response = await axios.post(
			`${baseUrl}/staff`,
			{
				id: 0, // or leave it out if the backend auto-generates IDs
				name: role.value.name,
				description: role.value.description,
				active: true,
				created_By: "e9fc1bff-8c64-4660-817b-06f5efaa1d7e", // Replace with the actual value
				created_At: new Date().toISOString(),
				updated_By: "e9fc1bff-8c64-4660-817b-06f5efaa1d7e", // Replace with the actual value
...
const handleEditSubmit = async () => {
	try {
		await axios.put(
			`${baseUrl}/staff/${selectedRole.value.id}`,
			selectedRole.value,
			{
				headers: { Authorization: `Bearer ${getToken()}` },
			}
