<template>
	<div class="bg-white shadow-md h-screen rounded-lg">
		<p class="p-5 text-lg font-semibold underline text-[#00992B] moul-regular">ការបើកប្រាក់ខែបុគ្គលិក</p>
			...
			<table class="w-full table-radius text-sm text-left text-gray-500">
				...
				<tbody>
					<tr v-for="(staff, index) in paginatedItems" :key="staff.id"
						...
						<td class="px-6 border whitespace-nowrap overflow-hidden border-gray-200 py-3 ">{{
							staff.staff.baseSalary }} <span class="px-2">$</span></td>
						<td class="px-6 border whitespace-nowrap overflow-hidden border-gray-200 py-3 ">{{
							staff.gasoline }} <span class="px-2">$</span></td>
						<td class="px-6 border whitespace-nowrap overflow-hidden border-gray-200 py-3 ">{{
							staff.overtimePay }} <span class="px-2">$</span></td>
						<td class="px-6 border whitespace-nowrap overflow-hidden border-gray-200 py-3 ">{{ staff.bonuses
							}} <span class="px-2">$</span></td>
						<td
							class="px-6 border whitespace-nowrap overflow-hidden text-[#00992B] font-bold border-gray-200 py-2">
							{{ new Date(staff.date).toLocaleDateString() }}</td>
						<td
...
const fetchStaffs = async () => {
	try {
		const token = getToken();
		const response = await axios.get(`${baseUrl}/staff/salary/Bonus`, {
			headers: { Authorization: `Bearer ${token}` },
		});
