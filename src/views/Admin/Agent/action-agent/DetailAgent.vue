					...
					<form @submit.prevent="handleSubmit">
						<div class="flex flex-wrap">
							...
							<div class="w-full lg:w-6/12 xl:w-4/12 px-2 mb-4">
								...
								<input type="number" v-model="form.commission" required disabled
									class="border px-3 py-2 placeholder-blueGray-300 text-blueGray-600 bg-white rounded text-sm focus:outline-none focus:ring w-full" />
							</div>
...
const route = useRoute(); // For getting agent ID from the route
...
const fetchAgentById = async () => {
    ...
    try {
        const response = await axios.get(`${baseUrl}/agent/${agentId}`);
        const agentData = response.data;

        form.value.firstName = agentData.firstName;
        form.value.lastName = agentData.lastName;
        form.value.enName = agentData.enName;
        form.value.email = agentData.email;
        form.value.dateOfBirth = agentData.dateOfBirth.slice(0, 10);
        form.value.commission = agentData.commission;
        form.value.gender = agentData.gender;
        form.value.marriedStatus = agentData.marriedStatus;
        form.value.phone1 = agentData.phone1;
        form.value.phone2 = agentData.phone2;
        form.value.status = agentData.status;
        form.value.positionId = agentData.position.id;
        form.value.branchId = agentData.branch.id;

        // Set location fields based on names
        form.value.provinceName_kh = provinces.value.find(p => p.name_kh === agentData.provinceName)?.code || "";
        await filterDistricts();
        form.value.districtName_kh = districts.value.find(d => d.name_kh === agentData.districtName)?.code || "";
        await filterCommunes();
        form.value.communeName_kh = communes.value.find(c => c.name_kh === agentData.communeName)?.code || "";
        await filterVillages();
        form.value.villageName_kh = villages.value.find(v => v.name_kh === agentData.villageName)?.code || "";
...
const handleSubmit = async () => {
	try {
		...
		await axios.patch(`${baseUrl}/agent/${route.params.id}`, payload);
		router.push("/agent");
